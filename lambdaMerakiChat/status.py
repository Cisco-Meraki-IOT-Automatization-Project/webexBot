...
def get_organizations2(session, api_key):
...
def create_network(session, api_key, org_id, name=None, type=None, tags=None, timeZone=None,
    ...
    variables = ['name', 'type', 'tags', 'timeZone', 'copyFromNetworkId', 'disableMyMerakiCom',
...
def device_status(session, headers, payload, api_key):
    ...
    for org in orgs:
        # post_message(session, headers, payload, org)
        print(org)
...
def get_alertas(api_key, message):
    headers2 = {'X-Cisco-Meraki-API-Key': api_key}
    r1 = requests.get(f"{_bURL}/networks/L_770115536280355076/alerts/settings", headers=headers2)
    alertas = []
    final = []
    for alerta in r1.json()['alerts']:
        if (alerta['type'] == 'sensorAlert'):
            alertas.append(alerta)
    filter = message.split('--filter ')
    if len(filter) == 1:
        return alertas
    else:
        ...
        for alert in alertas:
            final.append('--')
            final.append(alert['filters'][filter[1]])

        return final

def get_alerts(api_key, network_id):
    headers = {'X-Cisco-Meraki-API-Key': api_key}
    r1=requests.get(f"{_bURL}/networks/"+network_id+"/alerts/settings",headers=headers)
    alerts=json.loads(r1.content.decode('utf-8'))['alerts']
    response=[]
    for alerta in r1.json()['alerts']:
        if(alerta['type']=='sensorAlert'):
